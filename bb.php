...
* @author Ejaj_Mahmud < h4ck3r.emj@gmail.com >
* 
*/
...
";
	...
	public function __action()
	{
		...
		if(preg_match("/([0-9]{10,16})/",$read))
		{
			...
			$post = str_replace("{ejaj_nomerHP}",$read,self::$API_URL[$api]['postdata']);
			if(preg_match("/".self::$API_URL[$api]['success']."/",self::__Boom(self::$API_URL[$api]['url'],$post)))
			{
				echo "[$i][SUCCESS] ".self::__rapi($api,10," ")." :: $read ./ejaj ".PHP_EOL;
			}else{
				echo "[$i][FAILED ] ".self::__rapi($api,10," ")." :: $read ./ejaj".PHP_EOL;
			}
			sleep($delay);
			}
		}elseif(preg_match("/txt/",$read) || is_file($read))
		{
			...
			foreach($nlist as $nmr)
			{
				...
				{
					$api = array_rand(self::$API_URL);
					$post = str_replace("{ejaj_nomerHP}",$nmr,self::$API_URL[$api]['postdata']);
					if(preg_match("/".self::$API_URL[$api]['success']."/",self::__Boom(self::$API_URL[$api]['url'],$post)))
					{
						echo "[$i][SUCCESS] ".self::__rapi($api,10," ")." :: $nmr ./ejaj ".PHP_EOL;
					}else{
						echo "[$i][FAILED ] ".self::__rapi($api,10," ")." :: $nmr ./ejaj".PHP_EOL;
					}
					sleep($delay);
			}
			}
